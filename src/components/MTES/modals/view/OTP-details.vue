        ...
        <div class="panel">
            <div class="appdetails">
                ...
                <div class="appdetails1">
                    <p>Application Details{{ detail_id66 }} </p>
                </div>
                <div class="appdetails2">
                    <div class="appdetails3">
                        <p class="detailss">Application Number: <a style="font-weight: 400;">{{ detail_id6 }}</a></p>
...
export default { 
    ...
    methods: {
        Exit() {
            viewDetail6.value = false
        },
        async fetchDetails() {
            try {
                const response = await axios.get('http://127.0.0.1:8000/get_details/');
                this.details = response.data.find(det => det.id === parseInt(this.detail_id6))

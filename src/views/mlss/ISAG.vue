      ...
      <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          <tr v-for="(isag, index) in filteredISAG" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            ...
            <td class="px-6 py-4">{{ isag.released }}</td>
            <td class="px-6 py-4">{{ isag.status }}</td>
            <td class="px-6 py-4">{{ isag.remarks }}</td>
          </tr>
        </tbody>
      </table>
      <div v-if="filteredISAG.length === 0" class="text-center mt-4 text-gray-600 dark:text-gray-300">
        No results found.
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    fetchISAG() {
      axios.get('http://localhost:8000/api/isag')
        .then(response => {
    ...
    addNewEntry() {
      axios.post('http://localhost:8000/api/isag', this.newEntry)
        .then(response => {

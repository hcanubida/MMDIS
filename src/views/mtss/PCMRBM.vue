  ...
  <div>
    ...
    <div class="flex bg-white justify-between pl-4 pt-4">
      <h2 class="flex text-xl font-semibold">
        The total sum of PCMRB Meetings accomplished for the year {{ year }} is {{ totalSum }}.
      </h2>
    </div>
      ...
      <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
          <tr>
            ...
            <th scope="col" class="px-6 py-3">P/CMRB Meetings conducted/participated</th>
            <th scope="col" class="px-6 py-3 cursor-pointer" @click="sortByDate('date')">
              Date of Meeting
              <span v-if="sortKey === 'date'" aria-label="Sorted ascending">
        ...
        <tbody>
          <tr v-for="(entry, index) in filteredEntries" :key="entry.no" class="bg-white border-b">
            ...
            <td class="px-6 py-4">{{ entry.pcmrbm_meetings }}</td>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div class="mt-2 flex justify-between">
                    <p class="mr-5">P/CMRB Meetings conducted/participated:</p>
                    <input v-model="newEntry.pcmrbm_meetings" type="text" class="w-72 bg-orange-100 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                  </div>
                  <div class="mt-2 flex justify-between">
                    <p class="mr-5">Date of Meeting:</p>
                    <input v-model="newEntry.date" type="date" class="pl-1 pr-1 bg-orange-100 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                  </div>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div class="mt-2 flex justify-between">
                    <p class="mr-5">P/CMRB Meetings conducted/participated:</p>
                    <input v-model="updateEntry.pcmrbm_meetings" type="text" class="w-72 bg-orange-100 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                  </div>
                  <div class="mt-2 flex justify-between">
                    <p class="mr-5">Date  Meeting:</p>
                    <input v-model="updateEntry.date" type="date" class="pl-1 pr-1 bg-orange-100 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                  </div>
                  <div class="mt-4 flex justify-between">
                    <p class="mr-5">Proof of MOV Uploaded:</p>
                    <input ref="MOVpdf" type="file" accept="application/pdf" class="w-72 bg-orange-100 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
...
export default {
  ...
  data() {
    return {
      pcmrbm: [], // Array to store fetched entries
  ...
  computed: {
    ...
    totalSum() {
      const latestYear = Math.max(...this.pcmrbm.map(item => new Date(item.date).getFullYear()));
      return this.pcmrbm
        .filter(pcmrbm => new Date(pcmrbm.date).getFullYear() === latestYear)
        .length;
    },
    monthlyTotals() {
      const latestYear = Math.max(...(this.pcmrbm || []).map(item => new Date(item.date).getFullYear()));
  ...
  methods: {
    ...
    fetchDMPF() {
      axios.get('http://localhost:8000/api/MonitoringPCMRBM')
        ...
        .catch(error => {
          console.error('Error fetching monitoringPCMRBM:', error);
        });
    },
    ...
    addNewEntry() {
      ...
      formData.append('pcmrbm_meetings', this.newEntry.pcmrbm_meetings);
    ...
    openUpdateModal(entryNo) {
      const entry = this.pcmrbm.find(e => e.no === entryNo); // Find the entry to be updated by its number
      if (entry) {
    ...
    handleUpdate() {
      ...
      axios.put(`http://localhost:8000/api/MonitoringPCMRBM/${updatedEntry.no}`, updatedEntry)
        .then(response => {
          ...
          if (index !== -1) {
            this.pcmrbm[index] = response.data; // Directly assign the updated data to the entry in the array
          }
